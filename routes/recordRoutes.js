...
// POST route to view record data based on CNIC
router.post("/viewrecordInfo", async (req, res) => {

  try {
    const { error, value } = await recordFormValidationSchema.extract('cnic').validate(req.body.cnic);
    if (error) {
    ...
    const recordData = {
      ...record.toObject(),
      images: record.images ? record.images.map(imagePath => `${process.env.SERVER_URI}${imagePath}`) : [],
    };
    res.status(200).json({ message: "record found.", data: recordData });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "There was an error fetching the record data." });
  }
});
